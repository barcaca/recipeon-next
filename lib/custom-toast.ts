  ...
  setTimeout(() => {
    switch (data.status) {
      ...
      case 204:
        ...
        break
      case 400:
        toast.error(data.message || 'Solicitação inválida. Verifique os dados fornecidos.')
        break
      case 401:
        toast.error(data.message || 'Não autorizado. Verifique suas credenciais.')
        break
      case 403:
        toast.error(data.message || 'Proibido. Você não tem permissão para realizar esta ação.')
        break
      case 404:
      ...
      case 409:
        ...
        break
      case 429:
        toast.warning(data.message || 'Muitas solicitações. Tente novamente mais tarde.')
        break
      case 500:
        toast.error(data.message || 'Erro interno no servidor. Tente novamente mais tarde.')
        break
      case 503:
        toast.warning(data.message || 'Serviço indisponível. Tente novamente mais tarde.')
        break
      default:
